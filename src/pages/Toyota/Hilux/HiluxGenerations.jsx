import './HiluxGenerations.css';
import { Helmet } from 'react-helmet-async';

const HiluxGenerations = () => {
  return (
    <div className="hilux-container">
      <Helmet>
          <meta charSet="utf-8" />
          <title>Explore Toyota Hilux Generations in Kenya | KenyanAuto</title>
          <meta
              name="description"
              content="Explore Toyota Hilux Generations in Kenya" />
          <link rel="canonical" href="https://kenyanauto.vimtec.co.ke/Toyota/hilux" />
      </Helmet>
      <header className="hilux-header">
        <h1>Toyota Hilux: A Legacy of Durability Across Generations</h1>
        <p>
          The Toyota Hilux, a mid-size pickup produced by Toyota since 1968, has gained global recognition for its robustness and reliability. Over eight generations, the Hilux has adapted to changing needs while maintaining its core values.
        </p>
      </header>

      <section className="hilux-generations">
        <h2 className="generations-title">Explore Hilux Generations</h2>

        {/* 1st Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/1st%20gen/Cover.webp?updatedAt=1746441864598" alt="Toyota Hilux 1st Generation" className="generation-image" />
          <div className="generation-info">
            <h3>1st Generation (1968-1972)</h3>
            <p>
              The foundation of the Hilux legacy, offering durability and practicality. Click below to learn more about this groundbreaking model.
            </p>
            <a href="/Toyota/hilux-1st-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 2nd Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/2nd%20gen/Cover.webp?updatedAt=1746441864535" alt="Toyota Hilux 2nd Generation" className="generation-image" />
          <div className="generation-info">
            <h3>2nd Generation (1972-1978)</h3>
            <p>
              Enhanced design and improved performance made this generation a favorite among users. Discover its advancements below.
            </p>
            <a href="/Toyota/hilux-2nd-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 3rd Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/3rd%20gen/Cover.webp?updatedAt=1746441864509" alt="Toyota Hilux 3rd Generation" className="generation-image" />
          <div className="generation-info">
            <h3>3rd Generation (1978-1983)</h3>
            <p>
              Featuring the introduction of a double-cab variant, this generation expanded the Hilux's appeal. Learn more below.
            </p>
            <a href="/Toyota/hilux-3rd-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 4th Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/4th%20gen/Cover.webp?updatedAt=1746441865919" alt="Toyota Hilux 4th Generation" className="generation-image" />
          <div className="generation-info">
            <h3>4th Generation (1983-1988)</h3>
            <p>
              Expanded model range and enhanced reliability characterized this generation. Dive deeper below.
            </p>
            <a href="/Toyota/hilux-4th-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 5th Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/5th%20gen/Cover.webp?updatedAt=1746441864577" alt="Toyota Hilux 5th Generation" className="generation-image" />
          <div className="generation-info">
            <h3>5th Generation (1988-1997)</h3>
            <p>
              The introduction of the 1.5-cab variant marked this generation. Explore the details below.
            </p>
            <a href="/Toyota/hilux-5th-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 6th Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/6th%20gen/Cover.webp?updatedAt=1746441865917" alt="Toyota Hilux 6th Generation" className="generation-image" />
          <div className="generation-info">
            <h3>6th Generation (1997-2006)</h3>
            <p>
              Restyling and a wide variety of cab styles defined this era. Dive into its story below.
            </p>
            <a href="/Toyota/hilux-6th-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 7th Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/7th%20gen/Cover.webp?updatedAt=1746441864766" alt="Toyota Hilux 7th Generation" className="generation-image" />
          <div className="generation-info">
            <h3>7th Generation (2004-2015)</h3>
            <p>
              Featuring modern design and advanced features, this generation set new benchmarks. Learn more below.
            </p>
            <a href="/Toyota/hilux-7th-gen" className="learn-more-link">Read More</a>
          </div>
        </div>

        {/* 8th Generation */}
        <div className="generation-card">
          <img src="https://ik.imagekit.io/janjez/KenyanAuto/Toyota/Hilux/8th%20gen/Cover.webp?updatedAt=1746441864639" alt="Toyota Hilux 8th Generation" className="generation-image" />
          <div className="generation-info">
            <h3>8th Generation (2015-Present)</h3>
            <p>
              Combining cutting-edge technology with legendary toughness, the latest Hilux remains a global icon. Explore the details below.
            </p>
            <a href="/Toyota/hilux-8th-gen" className="learn-more-link">Read More</a>
          </div>
        </div>
      </section>
    </div>
  );
};

export default HiluxGenerations;
